...
# ... (restante do arquivo sem outro @app.route("/") duplicado)
    ...
    if request.method == "POST":
        try:
            p = Produto(
                ...
                preco_venda_sugerido=float(request.form["preco_venda_sugerido"] or 0),
...
def excluir_movimentacao(id):
    ...
    db.session.commit()
    flash("Movimentação removida.", "info")
...
def editar_produto(id):
    ...
    if request.method == "POST":
        ...
        produto.preco_venda_sugerido = float(request.form["preco_venda_sugerido"] or 0)
